...
export default function TableRow({data}) {
    ...
    const {formatTimeStamp,statusConfig,progressColor} = commonConfig()
    ...
    const handleSelectBaggerQuotationData = (baggerQuotationId) => {

        getBaggerQuotationById(baggerQuotationId)
            ...
            <th className="flex gap-3 px-6 py-4 font-normal text-gray-900">
                <div className="text-sm">
                    <div className="font-medium text-gray-700">
                        <Link to={'/quotations/data'} onClick={() => handleSelectBaggerQuotationData(id)} className='underline hover:text-indigo-600'>{consecutive}</Link>
                    </div>
                </div>
            </th>
