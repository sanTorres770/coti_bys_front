export default function InputForm({inputId, reference, defaultValue, labelValue, validationErrors, type, onChangeFunction, textUppercase, additionalLabelLink, placeholder, readonly, onFocusFunction, preSpan, onBlurFunction}) {
    return (
        <div>
            ...
            <div className={`flex w-full rounded-md ${validationErrors ? 'ring-red-600' : ''} shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600`}>
                ...
                <input id={inputId ? inputId : null}
                       type={type}
                       className={`block flex-1 placeholder:font-light border-0 bg-transparent py-1.5 pl-1 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm sm:leading-6`}
            ...
            {validationErrors && (<p className='text-sm text-red-600 mt-1'>{validationErrors}</p>)}
